...
def send_votes(num_votes: int, delay: Union[int, float], delay_offset_range: Union[float, int] = 0) -> None:
    print('Sending votes')
    votes_so_far = 0
    try:
                    ...
                    while True:
                        try:
                            ...
                            if error_ct > 5:
                                ...
                                sleep(.5)
                    submit.click()
                    browser.find_element(By.XPATH, value='/html/body/div[1]/div[2]/div[1]/div/div[4]/a[2]').click()
                    counter += 1
                    votes_so_far += 1
                    reloads_in_a_row = 0
...
def day(times: List[int], num_votes: Tuple[int, int], timing_offset_range: Union[float, int] = 0) -> None:
    if now() > times[-1]:
